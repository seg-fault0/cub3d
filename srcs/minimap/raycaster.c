...
#define TILE_SIZE       20
#define MINI_CENTER_X   210
#define MINI_CENTER_Y   110
#define MINI_MAX_X      390
#define MINI_MAX_Y      210
#define MINI_SCALE      TILE_SIZE

static bool	ray_mm_border(t_fvector2 pos, t_fvector2 ray)
{
	t_fvector2	screen;

	screen.x = (ray.x - pos.x) * MINI_SCALE + MINI_CENTER_X;
	screen.y = (ray.y - pos.y) * MINI_SCALE + MINI_CENTER_Y;
	if (screen.x >= MINI_MAX_X || screen.y >= MINI_MAX_Y
...
static void	draw_ray(t_cub *cub, float angle)
{
		...
		&& cub->parse.map[(int)ray.y][(int)ray.x] != 'C')
	{
		...
		put_pixel_to_img(&cub->img.display,
			(ray.x - pos.x) * MINI_SCALE + MINI_CENTER_X,
			(ray.y - pos.y) * MINI_SCALE + MINI_CENTER_Y,
			RAY_COLOR);
