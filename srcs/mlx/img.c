...
/*   Updated: 2025/09/10 11:25:55 by wimam            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*mlx_get_img(void *mlx, char *path, int *with, int *height)
{
	int	size;

	return (mlx_xpm_file_to_image(mlx, path, with, height));
}
...
bool	img_init(t_cub *cub)
{
	...
	cub->img.mm_frame.p = mlx_get_img(cub->mlx.mlx, MM_FRAME, &cub->img.mm_frame.with, &cub->img.mm_frame.height);
	if (!cub->img.mm_frame.p)
